...
function run(cmd) {
  ...
  if (internalCmd) {
    ...
    try {
      ...
      rawoutput = sandboxframe.contentWindow.eval(cmd); // here send to sandbox
      //if (typeof(rawoutput) == "object" && typeof(rawoutput.toString) == "function") {
      //    rawoutput = rawoutput.toString();
      //}
      if (typeof(rawoutput) == "undefined") { //hacked to get the content 
        rawoutput = "No value return, check debug for content changed";
        console.log("sandboxframe", sandboxframe.contentDocument.body);
      }
    } catch (e) {
      rawoutput = e.message;
      className = 'error';
...
function showhelp() {
  var commands = [
    '__ WORK BEST IN CHROME __',
    ':load &lt;url&gt; - to inject new DOM',
    ':load &lt;script_url&gt; - to inject external library',
    '      load also supports following shortcuts: <br />      jquery (preloaded), crypto (preloaded), <a href="https://github.com/pgriess/node-jspack">jspack</a>, underscore, prototype, mootools, dojo, rightjs, coffeescript, yui.<br />      eg. :load jquery',
    //':listen [id] - to start <a href="remote-debugging.html" target="_black">remote debugging</a> session',
    ':clear - to clear the history (accessed using cursor keys)',
    ':history - list current session history',
    ':about',
    'For Crypto: cc.md5("test") or "test".md5(),  <a href="http://code.google.com/p/crypto-js" target="_blank"> See here for more </a>' ,
    '- Supported String object: md5, sha, tohex, tobhex, urlen, urlde, rot13 -',
    '',
    '*BETA* :load pen.js , pen.get(url,str): send http request to url and \'grep str\' as result',
...
function about() {
  return 'Built by <a target="_new" href="http://twitter.com/rem">@rem</a>, crypto from http://code.google.com/p/crypto-js, -- hacked by w00d --';
}
